	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.juanzengbiaoti" placeholder="捐赠标题"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('juanzengwupin','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('juanzengwupin','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('juanzengwupin','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('juanzengwupin','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					...
					<template #default="scope">
						{{scope.row.juanzengbiaoti}}
					</template>
				</el-table-column>
				<el-table-column label="捐赠图片" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<div v-if="scope.row.juanzengtupian">
							<el-image v-if="scope.row.juanzengtupian.substring(0,4)=='http'" preview-teleported
								:preview-src-list="[scope.row.juanzengtupian.split(',')[0]]"
								:src="scope.row.juanzengtupian.split(',')[0]" style="width:100px;height:100px"></el-image>
							<el-image v-else preview-teleported
								:preview-src-list="[$config.url+scope.row.juanzengtupian.split(',')[0]]"
								:src="$config.url+scope.row.juanzengtupian.split(',')[0]" style="width:100px;height:100px">
							</el-image>
						</div>
						<div v-else>无图片</div>
					</template>
				...
				<el-table-column label="审核" v-if="btnAuth('juanzengwupin','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('juanzengwupin','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.juanzengbiaoti&&searchQuery.value.juanzengbiaoti!=''){
			params['juanzengbiaoti'] = '%' + searchQuery.value.juanzengbiaoti + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			id:row.id,
			juanzengbiaoti: row.juanzengbiaoti,
			juanzengtupian: row.juanzengtupian,
