	...
	<div>
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="12">
						<el-form-item label="捐赠标题" prop="juanzengbiaoti">
							<el-input class="list_inp" v-model="form.juanzengbiaoti" placeholder="捐赠标题"
								 type="text" 								:readonly="!isAdd||disabledForm.juanzengbiaoti?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="捐赠图片" prop="juanzengtupian">
							<uploads
								:disabled="!isAdd||disabledForm.juanzengtupian?true:false"
								action="file/upload" 
								tip="请上传捐赠图片" 
								:limit="3" 
								style="width: 100%;text-align: left;"
								:fileUrls="form.juanzengtupian?form.juanzengtupian:''" 
								@change="juanzengtupianUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const disabledForm = ref({
		juanzengbiaoti : false,
		juanzengtupian : false,
		xingming : false,
		zhanghao : false,
		shouji : false,
		sfsh : false,
	...
	const type = ref('')
	//捐赠图片上传回调
	const juanzengtupianUploadSuccess=(e)=>{
		form.value.juanzengtupian = e
	}
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='juanzengbiaoti'){
					form.value.juanzengbiaoti = row[x];
					disabledForm.value.juanzengbiaoti = true;
					continue;
				}
				if(x=='juanzengtupian'){
					form.value.juanzengtupian = row[x];
					disabledForm.value.juanzengtupian = true;
					continue;
				}
				if(x=='xingming'){
	...
	//提交
	const save=()=>{
		if(form.value.juanzengtupian!=null) {
			form.value.juanzengtupian = form.value.juanzengtupian.replace(new RegExp(context?.$config.url,"g"),"");
		}
