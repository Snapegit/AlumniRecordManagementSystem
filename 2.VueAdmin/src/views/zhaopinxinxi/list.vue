	...
	<div>
		<div class="app-contain">
				...
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.zhaopinbiaoti" placeholder="招聘标题"
								clearable>
							</el-input>
						</div>
					</div>
			...
			<el-table
				...
				<el-table-column label="公司图片" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<div v-if="scope.row.gongsitupian">
							<el-image v-if="scope.row.gongsitupian.substring(0,4)=='http'" preview-teleported
								:preview-src-list="[scope.row.gongsitupian.split(',')[0]]"
								:src="scope.row.gongsitupian.split(',')[0]" style="width:100px;height:100px"></el-image>
							<el-image v-else preview-teleported
								:preview-src-list="[$config.url+scope.row.gongsitupian.split(',')[0]]"
								:src="$config.url+scope.row.gongsitupian.split(',')[0]" style="width:100px;height:100px">
							</el-image>
						</div>
						<div v-else>无图片</div>
					</template>
				...
				<el-table-column label="审核" v-if="btnAuth('zhaopinxinxi','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.zhaopinbiaoti&&searchQuery.value.zhaopinbiaoti!=''){
			params['zhaopinbiaoti'] = '%' + searchQuery.value.zhaopinbiaoti + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			...
			jingyan: row.jingyan,
			xueli: row.xueli,
			zhiweimiaoshu: row.zhiweimiaoshu,
			zhanghao: row.zhanghao,
			xingming: row.xingming,
			shouji: row.shouji,
			gongsitupian: row.gongsitupian,
			sfsh: row.sfsh,
			shhf: row.shhf,
		}
		nextTick(() => {
